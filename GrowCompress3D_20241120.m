function GrowCompress3D_20241120(slurmID,steps,rate_stdev,ang_stdev,Pcritical,density_factor)
...
fname5 = strcat(dataDir,'/',char(startTime2),'.csv');
...
firstLine = [rate_stdev, ang_stdev, Pcritical*1e-6, density_factor];
...
% Define the full file path for nodeCoordinates.csv
nodeCoordinatesFile = fullfile(dataDir, 'nodeCoordinates.csv');

% Write the nodeCoordinates matrix to the CSV file
% Assuming nodeCoordinates is a 2D matrix where rows are points and columns are x, y, z
writematrix(nodeCoordinates, nodeCoordinatesFile, 'Delimiter', ',');
